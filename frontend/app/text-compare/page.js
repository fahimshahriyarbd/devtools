...
// Language dropdown order (per user request):
//   1. plaintext
//   2. php, javascript, python, c, cpp, csharp   ← explicitly listed
//   3. other programming languages
//   4. markup / data languages (html, css, xml, yaml, json, markdown, …)
const LANGS = [
  'plaintext',
  // Explicit priority languages
  'php', 'javascript', 'python', 'c', 'cpp', 'csharp',
  // Other programming languages
  'typescript', 'java', 'go', 'rust', 'ruby', 'shell', 'sql',
  // Markup / data languages
  'html', 'css', 'scss', 'xml', 'yaml', 'json', 'ini', 'markdown',
];

// Human-friendly labels for the language dropdown. The map key is the
// Monaco language id (kept as-is when passed to the editor) and the
// value is what the user actually sees in the Select. Anything not
// listed here falls back to a capitalised version of the id.
const LANG_LABELS = {
...
export default function TextComparePage() {
  ...
  // On narrow viewports force inline mode (see comment in CompareShared).
  // Side-by-side + wrap on a phone produces huge view-zone gaps between rows.
  ...
  const effectiveSideBySide = renderSideBySide && !isNarrow;
  ...
  useEffect(() => {
    const apply = () => {
      ...
      const isInline = mode === 'diff' && !effectiveSideBySide;
  ...
  }, [wordWrap, mode, effectiveSideBySide]);
  ...
  const copyBase = () => {
    const val = mode === 'edit'
      ? (origEditorRef.current?.getValue() ?? originalRef.current)
      : snapshot.a;
    try { navigator.clipboard.writeText(val); } catch { /* noop */ }
    toast.success('Copied base', TOAST_POS);
  };
  ...
  const copyResult = () => {
    const val = mode === 'edit'
      ...
      : snapshot.b;
    try { navigator.clipboard.writeText(val); } catch { /* noop */ }
    toast.success('Copied compare', TOAST_POS);
  };
  ...
  // Clear the contents of one side (Original or Modified) — leaves the
  // OTHER pane untouched, unlike the toolbar "Clear" button which resets
  // both. Used by the per-pane Clear buttons on the editor header.
  const clearPane = useCallback((which) => {
    const editor = which === 'original' ? origEditorRef.current : modEditorRef.current;
    if (editor) {
      editor.setValue('');
      editor.focus();
    }
    if (which === 'original') originalRef.current = '';
    else modifiedRef.current = '';
    // Bump editVersion so the DiffEditor snapshot picks up the empty value
    // the next time the user clicks Compare.
    setEditVersion(v => v + 1);
    toast.success(`${which === 'original' ? 'Original' : 'Modified'} cleared`, { position: 'bottom-right' });
  }, []);
      ...
      <header className="flex flex-wrap items-center gap-3 p-4 border-b border-border/60 bg-card/40 backdrop-blur">
        ...
        <div className="ml-auto flex flex-wrap items-center gap-3">
          <Select value={language} onValueChange={setLanguage}>
            ...
            <SelectContent>{LANGS.map(l => <SelectItem key={l} value={l}>{LANG_LABELS[l] || l}</SelectItem>)}</SelectContent>
          </Select>
          ...
          <Button variant="outline" size="sm" onClick={copyBase} data-testid="copy-base-btn" title="Copy base (original) text">
            <Copy className="h-3.5 w-3.5 mr-1.5" />Base
          </Button>
          <Button variant="outline" size="sm" onClick={copyResult} data-testid="copy-compare-btn" title="Copy compare (modified) text">
            <Copy className="h-3.5 w-3.5 mr-1.5" />Compare
          </Button>
      ...
      <div className="flex-1 min-h-0 p-4">
        {mode === 'edit' ? (
          ...
          <Card className="glass h-full overflow-hidden diff-outer-scroll">
            <DiffEditor
              key={`${snapshot.key}-${effectiveSideBySide ? 'sbs' : 'inl'}-${isNarrow ? 'n' : 'w'}`}
              height="100%"
              original={snapshot.a}
              modified={snapshot.b}
              language={language}
              ...
              onMount={(ed) => {
                ...
                orig.updateOptions({ ...pad, scrollbar: { useShadows: false, horizontal: 'hidden', vertical: 'visible', verticalScrollbarSize: 8 } });
                mod.updateOptions({ ...pad, scrollbar: { useShadows: false, horizontal: 'visible', vertical: 'visible', verticalScrollbarSize: 8, horizontalScrollbarSize: 8 } });
                ...
                const value = wordWrap ? 'on' : 'off';
                const origTarget = effectiveSideBySide ? value : 'off';
...
function EditorPane({ title, value, language, theme, wordWrap, version, onMount, onChange, accent, onPaste, onClear, testId }) {
  return (
    <Card className="glass relative overflow-hidden flex flex-col h-full">
        ...
        <div className="flex items-center gap-2">
          ...
          {onClear && (
            <button
              type="button"
              onClick={onClear}
              data-testid={`clear-btn-${testId || title.toLowerCase()}`}
              className="inline-flex items-center gap-1 normal-case font-medium px-2 py-0.5 rounded-md border border-border/60 bg-background/40 text-foreground/80 hover:bg-rose-500/15 hover:text-rose-300 hover:border-rose-500/40 transition"
      ...
      <div className="flex-1 min-h-0">
        <Editor
          ...
          onMount={(ed, monaco) => {
            onMount?.(ed, monaco);
            // One-click dismiss for Monaco's suggestion (IntelliSense) widget:
            // any mousedown outside the widget itself immediately closes it,
            // so the same click also lands on whatever the user intended —
            // no second click needed. Clicks INSIDE the widget still accept
            // a suggestion (Monaco's default behaviour).
            const dismiss = (e) => {
              const t = e.target;
              if (t && t.closest && t.closest('.suggest-widget')) return;
              try { ed.trigger('outside-click', 'hideSuggestWidget', null); } catch { /* noop */ }
            };
            document.addEventListener('mousedown', dismiss, true);
            ed.onDidDispose?.(() => document.removeEventListener('mousedown', dismiss, true));
          }}
