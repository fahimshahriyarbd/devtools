...
export const TEXT_EXT = /\.(txt|md|json|js|jsx|ts|tsx|html|css|scss|less|xml|yaml|yml|toml|ini|env|conf|sh|bash|py|go|rs|rb|php|java|kt|swift|c|cc|cpp|cs|h|hpp|sql|csv|log|gitignore|gitattributes|prettierrc|eslintrc|babelrc|dockerfile)$/i;
...
export function RowItem({ row, selected, opened, onClick, onToggleSeen }) {
  ...
  return (
    <div
      ...
      className={cn('w-full text-left flex items-center gap-3 px-3 py-2 my-1.5 rounded-md text-sm font-mono group transition border cursor-pointer',
        selected
          ? 'bg-accent text-accent-foreground border-transparent'
          : opened
            ? 'bg-violet-500/25 hover:bg-violet-500/35 border-violet-400/60 shadow-sm shadow-violet-500/20'
            : 'border-transparent hover:bg-accent/50')}
    >
      <span className={cn('h-6 w-6 rounded-md grid place-items-center shrink-0', dotBg)}>
        <Icon className={cn('h-3.5 w-3.5', color)} />
      </span>
      <span className={cn('truncate flex-1', opened && !selected && 'text-violet-100')}>{row.path}</span>
      ...
      <span
        role="button"
        tabIndex={0}
        data-testid={`compare-row-seen-${row.path}`}
        aria-pressed={opened ? 'true' : 'false'}
        onClick={(e) => { e.stopPropagation(); onToggleSeen?.(row.path); }}
        onKeyDown={(e) => {
          if (e.key === 'Enter' || e.key === ' ') {
            e.preventDefault();
            e.stopPropagation();
            onToggleSeen?.(row.path);
          }
        }}
        className={cn(
          'text-[9px] font-sans uppercase tracking-wider rounded px-1.5 py-[1px] border cursor-pointer hidden sm:inline transition select-none',
          'text-white bg-violet-500 border-violet-400 shadow-sm hover:bg-violet-600'
        )}
        title={opened ? 'Click to mark as unseen' : 'Click to mark as seen'}
      >
        seen
      </span>
...
export function PreviewDialog({ row, prev, next, onPrev, onNext, onClose, theme, sideBySide, setSideBySide, ignoreWs, setIgnoreWs, wrap, setWrap, baseLabel = 'Base', compareLabel = 'Compare' }) {
  ...
  // On narrow viewports (phones), force the DiffEditor into inline mode even
  // if the user prefers side-by-side. In side-by-side, Monaco aligns rows
  // between panes by inserting view-zone gaps proportional to how many
  // visual lines each side wraps to — on a phone this manifests as huge
  // empty space between every line. Inline mode doesn't need this alignment
  // so wrap renders naturally.
  ...
  const effectiveSideBySide = sideBySide && !isNarrow;
  ...
  useEffect(() => {
    const apply = () => {
      ...
      const origVal = effectiveSideBySide ? value : 'off';
      const origOpts = {
  ...
  }, [wrap, effectiveSideBySide, row?.path]);
      ...
      <div className="flex-1 min-h-0 relative diff-outer-scroll">
        {isText ? (
          (row.status === 'modified' || row.status === 'unchanged') ? (
            <DiffEditor
              key={`${row.path}-${effectiveSideBySide ? 'sbs' : 'inl'}-${isNarrow ? 'n' : 'w'}`}
              height="100%"
              original={leftText || ''}
              modified={rightText || ''}
              language={lang}
              ...
              onMount={(ed) => {
                ...
                const orig = ed.getOriginalEditor();
                const mod = ed.getModifiedEditor();
                const baseScroll = { scrollbar: { useShadows: false, vertical: 'visible', verticalScrollbarSize: 8, horizontalScrollbarSize: 8 } };
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 min-h-0">
        <Editor
          ...
          options={{ readOnly: true, automaticLayout: true, minimap: { enabled: false }, fontSize: 13, wordWrap: wrap ? 'on' : 'off', scrollBeyondLastLine: false, scrollbar: { useShadows: false, verticalScrollbarSize: 8, horizontalScrollbarSize: 8 } }}
        />
      </div>
    </div>
  );
