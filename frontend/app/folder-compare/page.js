...
export default function FolderComparePage() {
  ...
  // Track paths the user has opened in the preview dialog so already-
  // reviewed rows can render with a visibly different background.
  const [openedPaths, setOpenedPaths] = useState(() => new Set());
  // Reset the "seen" state and any highlighted row whenever either
  // folder changes identity (cleared and re-added, or swapped for a
  // different folder). Otherwise rows from a completely new pair would
  // still render as previously opened.
  useEffect(() => {
    setOpenedPaths(new Set());
    setSelected(null);
  }, [left, right]);
  const markOpened = (path) => {
    setOpenedPaths(prev => {
  ...
  // Toggle a row's "seen" state from the inline pill on the row itself.
  // Clicking the pill should NOT open the file — the pill's own click
  // handler stops propagation, so this only runs when the user actually
  // wants to flip the seen flag.
  ...
  const openRow = (path) => { setSelected(path); markOpened(path); };
      ...
      <header className="flex flex-wrap items-center gap-3 p-4 border-b border-border/60 bg-card/40 backdrop-blur">
        ...
        {diff && (
          <div className="flex items-center gap-2 text-xs">
            <Badge className="bg-emerald-500/15 text-emerald-300 border-emerald-500/30 border hover:bg-emerald-500/25">+{diff.stats.added} added</Badge>
            <Badge className="bg-rose-500/15 text-rose-300 border-rose-500/30 border hover:bg-rose-500/25">−{diff.stats.removed} removed</Badge>
            <Badge className="bg-blue-500/15 text-blue-300 border-blue-500/30 border hover:bg-blue-500/25">~{diff.stats.modified} modified</Badge>
        ...
        <div className="ml-auto flex items-center gap-3">
          ...
          {(left || right) && (
            <Button
              data-testid="folder-clear-both-btn"
              size="sm"
              variant="outline"
              onClick={() => {
                setLeft(null);
                ...
                setSearch('');
                setFilter('all');
                toast.success('Cleared both folders');
              }}
              className="text-rose-300 border-rose-500/40 hover:bg-rose-500/10 hover:text-rose-200"
            >
        ...
        <ScrollArea className="flex-1 px-2 py-2">
          {!diff ? (
            ...
            <div className="max-w-5xl mx-auto px-2">
              {visible.map(r => <RowItem key={r.path} row={r} selected={selected === r.path} opened={openedPaths.has(r.path)} onClick={() => openRow(r.path)} onToggleSeen={toggleSeen} />)}
        ...
        >
          ...
          {selectedRow && (
            <PreviewDialog
              ...
              prev={prev}
              next={next}
              onPrev={() => prev && openRow(prev.path)}
              onNext={() => next && openRow(next.path)}
