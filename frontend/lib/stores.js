  ...
  persist(
    (set, get) => ({
      ...
      addHistory: (entries) => set(s => {
        // Dedupe by (fullInput, value): if an incoming entry matches an
        // existing one on BOTH the source text and the hash value, we keep
        // only the incoming (newer) one and drop the older instance so the
        // recent entry gets promoted to the top. This also dedupes within
        // the incoming batch itself (later entries in the batch win).
        const keyOf = (e) => `${e?.fullInput ?? e?.preview ?? ''}\u0000${e?.value ?? ''}`;
        const incomingKeys = new Set();
        // Walk incoming in reverse so the LAST occurrence in the batch wins
        // (matches the "last generated sits at the top" rule when the same
        // hash is produced multiple times in one compute call).
        const dedupedIncoming = [];
        for (let i = entries.length - 1; i >= 0; i--) {
          const k = keyOf(entries[i]);
          if (incomingKeys.has(k)) continue;
          incomingKeys.add(k);
          dedupedIncoming.unshift(entries[i]);
        }
        const filteredExisting = s.history.filter(h => !incomingKeys.has(keyOf(h)));
        return { history: [...dedupedIncoming, ...filteredExisting].slice(0, 500) };
      }),
